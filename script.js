  ...
  <div class="bill">
    <div class="billtop">
      <div>
        ...
        <div class="billmeta">
          ${esc(b.category||"General")} · ${esc(b.frequency)}
        </div>
      </div>
    ...
    <div class="billbottom">
      ...
      <div class="row">
        ...
        <button class="btn mini danger" onclick="deleteBill('${b.id}')">Delete</button>
      </div>
    </div>
  </div>
  `;
    ...
    <div style="margin-top:10px">
      ${
        p.items.slice(0,7).map(x => `
          <div class="payrow">
            <span>${esc(x.b.name)} · ${fmtDate(x.d)}</span>
...
function openModal(title, body, onSave, onDelete){
  ...
  m.innerHTML = `
    ...
    ${body}
    <div class="modalfoot">
      ${onDelete ? `<button class="btn danger" id="modalDelete">Delete</button>` : ""}
...
function billForm(b={}){
  ...
  <div class="grid two">
    ...
    <div class="field full">
      <label>Notes</label>
      <textarea id="fNotes" placeholder="Optional notes...">${esc(b.notes||"")}</textarea>
    </div>
  </div>
  `;
}
...
function showBill(b=null){
  openModal(
    ...
    b ? () => {
      ...
      state.payments = state.payments.filter(p => p.billId !== b.id);
      save();
      render();
      toast("Bill deleted");
      return true;
...
function deleteBill(id){
  let b = state.bills.find(x => x.id === id);
  if(!b) return;
...
function generateReport(){
  ...
  <div class="report">
    ...
    <p style="margin-top:22px;font-size:9px;color:#777">
      Generated by Redline · Your money. Your schedule. Your plan.
    </p>
  </div>
  `;
}
